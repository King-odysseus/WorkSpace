...
function DialogContent({ className, overlayClassName, children, showCloseButton = true, position = 'center', ...props }) {
  ...
  const layered = docked || centered
  const viewportStyle = useDockViewport(layered)
  const content = (
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        docked
          ...
          : 'pointer-events-auto absolute top-1/2 left-1/2 grid w-[min(440px,calc(100%-30px))] -translate-x-1/2 -translate-y-1/2 gap-5 rounded-2xl border border-border p-6 shadow-[0_20px_70px_rgb(7_26_45_/31%)]',
        className
      )}
      {...props}
    >
  ...
  return (
    <DialogPortal>
      ...
      {layered ? (
        <div
          data-slot={docked ? 'dialog-dock-layer' : 'dialog-viewport-layer'}
          className={docked ? 'dialog-dock-layer' : 'dialog-viewport-layer'}
          style={viewportStyle}
        >
          {content}
        </div>
