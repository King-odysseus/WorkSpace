...
import {
  Archive, ArrowUpRight, Brush, CalendarDays, Check, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight,
  Clock3, Copy, Filter, Hash, MessageSquare, Pause, Play, Plus, Square, Target, X,
} from 'lucide-react'
...
import {
  ...
  formatLastSeen, formatShiftClock, formatShiftDuration, getCsrfToken, readJsonResponse, sortMembersByRecentActivity, toDateKey,
} from '../lib/workspace-format.js'

function TeamBoardView({ tasks, members, projects = [], scope = 'all', onScopeChange, invitations, canManageMembers, onInvite, onComplete, onStatusChange, onOpenTask, onUpdateMemberRole, onRemoveMember, onCancelInvitation, onResendInvitation }) {
...
function TodayDashboard({ today, todayLabel, currentUserName, currentUserId, currentUserPresence, workspaceName, tasks, events, followUps, checkIns, workShifts, members, canManageMembers, onAddTask, onInvite, onOpenTask, onNavigate, onComplete, onStatusChange, onSubmitShift, onChangePresence }) {
  const isOpen = task => task.status !== 'done'
  ...
  const messageOnlineMember = member => {
    onNavigate('Chats')
    window.setTimeout(() => window.dispatchEvent(new CustomEvent('chat:direct', { detail: { memberId: member.id } })), 0)
  }
  ...
  return <section className="today-dashboard">
    <section className="today-hero"><div><p className="eyebrow">{todayLabel}</p><h1>{greeting}, {currentUserName.split(' ')[0]}</h1><p className="subtitle">Here is what needs your attention in {workspaceName}.</p></div><div className="today-actions"><DropdownMenu><DropdownMenuTrigger asChild><Button>Quick action <ChevronDown size={16} /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
      <DropdownMenuItem onSelect={onAddTask}><Plus size={17} /> Add task</DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onNavigate('Calendar')}><CalendarDays size={16} /> Add event</DropdownMenuItem>
      <DropdownMenuItem onSelect={() => onNavigate('Check-ins')}><MessageSquare size={16} /> Check in</DropdownMenuItem>
      {canManageMembers && <DropdownMenuItem onSelect={onInvite}><Plus size={16} /> Invite team member</DropdownMenuItem>}
