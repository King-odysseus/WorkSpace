...
function RichEditor({ value, onChange, compact = false, readOnly = false, hideToolbar = false }) {
  ...
  // The toolbar used to render uncontrolled native <select>s, which kept their
  // own picked value. Combo boxes are controlled, so the picks live here. The
  // command still runs against the editor's selection - command() refocuses the
  // editor first, and onCloseAutoFocus stops the trigger taking focus back.
  ...
  const addLink = () => {
    ...
    else toast.error('Links must start with http://, https://, mailto:, tel:, or /.')
  }
  ...
  return <div className={`rich-editor ${compact ? 'is-compact' : ''}`}>
    {!readOnly && !hideToolbar && <div className="rich-toolbar" role="toolbar" aria-label="Text formatting">
      <div className="rich-toolbar-row">
        ...
        <div className="rich-toolbar-group rich-toolbar-font"><span>Font</span><AppSelect className="rich-toolbar-select is-font" onCloseAutoFocus={event => event.preventDefault()} value={fontFamily} onChange={event => { setFontFamily(event.target.value); command('fontName', event.target.value) }} aria-label="Font family"><option value="Arial">Arial</option><option value="Calibri">Calibri</option><option value="Georgia">Georgia</option><option value="Times New Roman">Times New Roman</option><option value="Verdana">Verdana</option></AppSelect><AppSelect className="rich-toolbar-select is-size" onCloseAutoFocus={event => event.preventDefault()} value={fontSize} onChange={event => { setFontSize(event.target.value); command('fontSize', event.target.value) }} aria-label="Font size"><option value="1">10</option><option value="2">12</option><option value="3">14</option><option value="4">18</option><option value="5">24</option><option value="6">32</option><option value="7">48</option></AppSelect></div>
      ...
      <div className="rich-toolbar-row">
        <div className="rich-toolbar-group"><span>Paragraph</span><AppSelect className="rich-toolbar-select is-block" onCloseAutoFocus={event => event.preventDefault()} value={blockStyle} onChange={event => { setBlockStyle(event.target.value); command('formatBlock', event.target.value) }} aria-label="Paragraph style"><option value="p">Paragraph</option><option value="h1">Title</option><option value="h2">Heading 1</option><option value="h3">Heading 2</option><option value="blockquote">Quote</option><option value="pre">Code</option></AppSelect>{toolbarButton('Bulleted list', <List size={15} />, () => command('insertUnorderedList'), 'insertUnorderedList')}{toolbarButton('Numbered list', <ListOrdered size={15} />, () => command('insertOrderedList'), 'insertOrderedList')}{toolbarButton('Decrease indent', <IndentDecrease size={15} />, () => command('outdent'))}{toolbarButton('Increase indent', <IndentIncrease size={15} />, () => command('indent'))}</div>
...
function promptForImage(label) {
  ...
  if (entered === null) return ''
  const url = safeUrl(entered)
  if (!url) toast.error('Image links must start with http://, https://, or / .')
  return url
}
...
export function FilesWorkspaceView({ workspaceId, currentUserId, notificationDocumentId = null, onNotificationDocumentHandled }) {
  ...
  if (selected) {
    ...
    return <section className="workspace-view file-editor-view">{deleteTarget && <FileDeleteDialog target={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={() => confirmDelete().catch(error => setStatus(error.message || 'Delete failed.'))} />}{restoreTarget && <FileRestoreDialog target={restoreTarget} onCancel={() => setRestoreTarget(null)} onConfirm={() => confirmRestoreRevision().catch(error => setStatus(error.message || 'Restore failed.'))} />}
      ...
      <div className={`file-editor-layout ${(commentsOpen || shareOpen || historyOpen) ? 'has-review-panel' : ''}`}>
        ...
        {(commentsOpen || shareOpen || historyOpen) && <aside className="document-review-panel">{historyOpen && <><h3>Version history</h3><div className="document-revision-list">{revisions.map(revision => <article key={revision.id}><strong>{formatDateTime(revision.created_at)}</strong><span>{revision.created_by}</span>{canEdit && <button type="button" onClick={() => restoreRevision(revision).catch(() => setStatus('Could not restore that version.'))}>Restore</button>}</article>)}</div>{!revisions.length && <p className="workspace-inline-status">No earlier versions yet.</p>}</>}{shareOpen && <><h3>Share</h3><form onSubmit={shareDocument}><AppSelect className="w-full" name="shareUserId" value={shareUserId} onChange={event => setShareUserId(event.target.value)} required><option value="">Choose a team member</option>{members.map(member => <option key={member.id} value={member.id}>{[member.first_name, member.last_name].filter(Boolean).join(' ') || member.email}</option>)}</AppSelect><AppSelect className="w-full" value={sharePermission} onChange={event => setSharePermission(event.target.value)}><option value="view">Can view</option><option value="comment">Can comment</option><option value="edit">Can edit</option></AppSelect><button className="primary-button">Share</button></form>{shares.map(share => <div className="document-share-row" key={share.id}><strong>{share.user_name}</strong><span>{share.permission}</span></div>)}</>}{commentsOpen && <><h3>Review comments</h3><div className="document-comment-list">{comments.map(comment => <article className={comment.resolved ? 'resolved' : ''} key={comment.id}><strong>{comment.author_name}</strong><span>{formatDateTime(comment.created_at)}{comment.anchor?.slide ? `, Slide ${comment.anchor.slide}` : ''}</span><p><LinkedText text={comment.body} /></p>{canComment && <button type="button" onClick={() => resolveComment(comment)}>{comment.resolved ? 'Reopen' : 'Resolve'}</button>}</article>)}</div>{canComment ? <form onSubmit={addComment}><MentionPicker members={members} value={commentDraft} onChange={setCommentDraft} currentUserId={currentUserId}>{inputRef => <textarea ref={inputRef} value={commentDraft} onChange={event => setCommentDraft(event.target.value)} placeholder="Add a review comment..." />}</MentionPicker><button className="primary-button">Comment</button></form> : <p className="workspace-inline-status">View-only access does not allow comments.</p>}</>}</aside>}
      </div>
  ...
  >{deleteTarget && <FileDeleteDialog target={deleteTarget} onCancel={() => setDeleteTarget(null)} onConfirm={() => confirmDelete().catch(error => setStatus(error.message || "Delete failed."))} />}<div className="files-browser-heading"><div><p className="eyebrow">Workspace resources</p><h2>Files</h2><p>Browse documents, presentations, and uploads.</p></div><div className="files-create-actions"><button type="button" className="secondary-button" onClick={() => createDocument('document')}><FileText size={15} /> New document</button><button type="button" className="secondary-button" onClick={() => createDocument('presentation')}><Presentation size={15} /> New presentation</button><button type="button" className="secondary-button" onClick={() => createDocument('spreadsheet')}><Table2 size={15} /> New spreadsheet</button><label className="primary-button"><Upload size={15} /> Upload<input type="file" hidden multiple onChange={upload} /></label></div></div>{status && <Alert tone="info" compact className="files-status-alert">{status}</Alert>}<div className="files-browser-toolbar"><div className="files-categories">{[['all', 'All'], ['document', 'Documents'], ['presentation', 'Presentations'], ['spreadsheet', 'Spreadsheets'], ['file', 'Uploads']].map(([value, label]) => <button type="button" className={category === value ? 'active' : ''} onClick={() => setCategory(value)} key={value}>{label}</button>)}</div><label className="files-search"><Search size={15} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search files" aria-label="Search files" /></label><div className="files-view-switch"><button type="button" className={viewMode === 'grid' ? 'active' : ''} onClick={() => setViewMode('grid')} aria-label="Thumbnail view"><Grid3X3 size={16} /></button><button type="button" className={viewMode === 'table' ? 'active' : ''} onClick={() => setViewMode('table')} aria-label="Table view"><Table2 size={16} /></button></div></div>{viewMode === 'grid' ? <div className="files-thumbnail-grid">{items.map(item => <div className="files-thumbnail-tile" key={`${item.itemType}-${item.id}`}><button type="button" onClick={() => openItem(item)}><span className={`file-thumbnail file-thumbnail-${item.itemType}`}>{item.itemType === 'presentation' ? <Presentation size={38} /> : item.itemType === 'spreadsheet' ? <Table2 size={38} /> : <FileText size={38} />}</span><strong>{item.title}</strong><small>{typeLabel(item.itemType)} · {formatDate(item.modified)}</small></button>{item.itemType === 'file' && <button type="button" className="files-tile-delete" onClick={() => removeFile(item)} aria-label={`Delete ${item.title}`} title="Delete"><Trash2 size={14} /></button>}</div>)}</div> : <div className="files-table-wrap"><table className="files-table"><thead><tr><th>Name</th><th>Type</th><th>Owner</th><th>Modified</th><th>Size</th><th><span className="visually-hidden">Actions</span></th></tr></thead><tbody>{items.map(item => <tr key={`${item.itemType}-${item.id}`} tabIndex={0} role="button" aria-label={`Open ${item.title}`} onClick={() => openItem(item)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openItem(item) } }}><td><span>{item.itemType === 'presentation' ? <Presentation size={17} /> : item.itemType === 'spreadsheet' ? <Table2 size={17} /> : <FileText size={17} />}</span>{item.title}</td><td>{typeLabel(item.itemType)}</td><td>{item.owner ? [item.owner.first_name, item.owner.last_name].filter(Boolean).join(' ') || item.owner.email : '-'}</td><td>{formatDateTime(item.modified)}</td><td>{item.size ? `${Math.ceil(item.size / 1024)} KB` : '-'}</td><td>{item.itemType === 'file' && <button type="button" className="files-row-delete" onClick={event => { event.stopPropagation(); removeFile(item) }} aria-label={`Delete ${item.title}`} title="Delete"><Trash2 size={15} /></button>}</td></tr>)}</tbody></table></div>}{!items.length && <div className="files-empty">No matching files.</div>}</section>
}
...
export function AISettingsPanel({ workspaceId, members = [], canManageMembers }) {
  ...
  const providerMeta = {
    openai: { label: 'OpenAI', description: 'Fast general model, best for summaries and drafting.' },
    claude: { label: 'Anthropic · Claude', description: 'Strong at long-context analysis and review.' },
    kimi: { label: 'Kimi', description: 'Long-context research for document-heavy work.' },
    deepseek: { label: 'DeepSeek', description: 'Efficient reasoning for technical workspace tasks.' },
  }
  ...
  const save = async () => {
    ...
    try {
      const response = await fetch(`/api/workspaces/${workspaceId}/ai/settings/`, {
        ...
        body: JSON.stringify({
          ...settings,
          provider_config: Object.fromEntries(
            AI_PROVIDERS.map(([provider]) => [provider, providerPayload(provider)]),
          ),
        }),
      })
      ...
      setData(current => ({
        ...
        savedProviderConfig: result.provider_config,
      }))
  ...
  return <Card className="settings-panel ai-settings-panel">
    <header className="ai-settings-heading">
      <div>
        <h2>AI assistance</h2>
        <p>Configure the providers available to this workspace and control who can use the assistant. Provider keys are stored encrypted and never exposed to members.</p>
      </div>
      <button
        type="button"
        className="ai-settings-help-toggle"
    ...
    {helpOpen && <div className="ai-settings-help">
      <strong>Connect a provider</strong>
      <p>Create an API key in the provider's developer console, add it to the provider row, choose the model, then save. Default base URLs are already filled in for a compatible gateway or proxy.</p>
      <p>Railway environment variables still work as a fallback, but they are not required for workspace setup.</p>
    </div>}

    <div className="ai-settings-enable-row">
      <div>
        <strong>Enable AI assistance</strong>
        <span>Turns on the workspace assistant, AI summaries and provider-backed features.</span>
      </div>
      <button
        type="button"
        role="switch"
        className="ai-settings-switch"
    ...
    <section className="ai-settings-providers" aria-labelledby="ai-provider-heading">
      <h3 id="ai-provider-heading">Default provider</h3>
      <div className="ai-provider-list">
        {AI_PROVIDERS.map(([provider, fallbackLabel]) => {
          const config = providerConfig[provider] || {}
          const meta = providerMeta[provider] || { label: fallbackLabel, description: 'Provider-backed assistant capabilities.' }
          const enabled = settings.ai_enabled_providers.includes(provider)
          ...
          return <div className="ai-provider-block" key={provider}>
            <div className={`ai-provider-row ${isDefault ? 'is-selected' : ''}`}>
              ...
              <button
                ...
                aria-controls={`ai-provider-config-${provider}`}
                onClick={() => setExpandedProvider(current => current === provider ? '' : provider)}
              >
                <strong>{meta.label}{config.model ? ` · ${config.model}` : ''}</strong>
                <span>{meta.description}</span>
              </button>
              <button
                type="button"
                className={`ai-provider-status ${status.className}`}
                aria-expanded={isExpanded}
                aria-controls={`ai-provider-config-${provider}`}
                aria-label={`Configure ${meta.label}; ${status.label}`}
                onClick={() => setExpandedProvider(current => current === provider ? '' : provider)}
              >
                {status.label}
              </button>
    ...
    <section className="ai-settings-members" aria-labelledby="ai-members-heading">
      <div className="ai-settings-members-heading">
        <h3 id="ai-members-heading">Members allowed to use AI</h3>
        <p>Owners and managers always have access. Select the other team members who may use the company key.</p>
      </div>
      <div className="ai-member-list">
        {members.length ? members.map(member => {
          const name = [member.first_name, member.last_name].filter(Boolean).join(' ') || member.email
          const allowed = settings.ai_user_ids.includes(member.id)
          return <div className="ai-member-row" key={member.id}>
            <Avatar name={name} avatarUrl={member.avatar_url} presence={member.presence} className="ai-member-avatar" />
            <span className="ai-member-copy">
              <strong>{name}</strong>
            ...
            <button
              ...
              aria-label={`Allow ${name} to use AI`}
        ...
        }) : <p className="ai-members-empty">No workspace members are available.</p>}
      </div>
    </section>
