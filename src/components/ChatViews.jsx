...
import { PRESENCE_LABEL, formatRelativeActivityTime, getCsrfToken, toDateKey } from '../lib/workspace-format.js'
...
function PresenceDot({ presence }) {
  ...
  return <span className={`presence-dot presence-${presence} chat-name-presence`} title={PRESENCE_LABEL[presence] || presence} />
}
...
function ChatWorkspaceView({ viewType, data, workspaceId, currentUserId, onRefresh, onError, onConfirm, onNavigate }) {
  ...
  const presenceForConversation = conversation => {
    ...
    return data.members.find(member => String(member.id) === String(other?.id))?.presence || null
  }
