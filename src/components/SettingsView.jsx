...
const AISettingsPanel = lazy(() => import('./WorkspaceTools.jsx').then(module => ({ default: module.AISettingsPanel })))
...
function SettingsView({ theme, onSetTheme, sidebarCollapsed, onToggleSidebar, currentWorkspace, currentUserName, currentUserEmail, currentUserId, currentUserAvatarUrl, currentUserPresence, onProfileUpdated, canManageMembers, members, notifications, workspaceId, workspaces = [], defaultWorkspaceId, onSetDefaultWorkspace, taskTemplates = [], projectTemplates = [], projects = [], onRefresh }) {
  ...
  const toggleManagerPermission = async (member, key) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Permission could not be updated.')
  ...
  const changeMemberRole = async (member, role) => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Role could not be updated.')
      onRefresh?.()
    } catch (error) {
  ...
  const runLifecycleAction = async (targetWorkspaceId, method, path, confirmMessage) => {
    if (confirmMessage && !window.confirm(confirmMessage)) return
    setLifecycleBusy(true)
    setLifecycleError('')
    try {
      const response = await fetch(`/api/workspaces/${targetWorkspaceId}${path}`, { method, credentials: 'include', headers: { 'Content-Type': 'application/json', 'X-CSRFToken': await getCsrfToken(), 'X-Workspace-Id': String(targetWorkspaceId) } })
  ...
  const leaveWorkspace = (targetWorkspaceId, name) => runLifecycleAction(targetWorkspaceId, 'POST', '/leave/', `Leave ${name || 'this workspace'}? You will lose access until you are invited again.`)
  const archiveWorkspace = () => runLifecycleAction(workspaceId, 'POST', '/archive/', `Archive ${currentWorkspace?.name || 'this workspace'}? Members keep read access; only the owner can restore or permanently delete it.`)
  ...
  return <section className="workspace-view settings-view">
    ...
    <div className="settings-shell">
      ...
      <div className="settings-content">
        ...
        {section === 'ai' && <Suspense fallback={null}><AISettingsPanel workspaceId={workspaceId} members={members} canManageMembers={canManageMembers} /></Suspense>}
        {section === 'profile' && workspaces.length >= 1 && <Card className="settings-panel"><div className="settings-panel-heading"><div><p className="eyebrow">Workspace entry</p><h2>Default workspace</h2><p>Choose the workspace WorkSpace opens when you sign in, or leave one you no longer need.</p></div></div>{lifecycleError && <p className="auth-error" role="alert">{lifecycleError}</p>}<div className="grid gap-3 sm:grid-cols-2">{workspaces.map(workspace => <div key={workspace.id} className={`rounded-xl border p-4 text-left transition-colors ${workspace.id === defaultWorkspaceId ? 'border-primary bg-primary/5' : 'border-border hover:bg-surface-secondary'}`}><button type="button" onClick={() => workspace.status === 'active' && onSetDefaultWorkspace?.(workspace.id)} aria-pressed={workspace.id === defaultWorkspaceId} disabled={workspace.status !== 'active'} className="block w-full text-left"><strong className="block text-sm">{workspace.name}{workspace.status === 'archived' ? ' (archived)' : ''}</strong><span className="mt-1 block text-xs text-text-muted">{workspace.id === defaultWorkspaceId ? 'Default entry workspace' : workspace.status === 'active' ? 'Set as default' : 'Archived - cannot be a default'}</span></button>{workspace.role !== 'owner' && <button type="button" className="text-button mt-2" disabled={lifecycleBusy} onClick={() => leaveWorkspace(workspace.id, workspace.name)}>Leave workspace</button>}</div>)}</div></Card>}
        ...
        {section === 'templates' && <Card className="settings-panel"><div className="settings-panel-heading"><div><p className="eyebrow">Reusable setup</p><h2>Templates</h2><p>Create templates for repeatable tasks and projects, then apply them from the create forms.</p></div></div><div className="settings-template-section"><h3>Task templates</h3><form className="settings-template-form" onSubmit={createTaskTemplate}><label>Template name<input value={taskTemplateForm.name} onChange={event => setTaskTemplateForm(current => ({ ...current, name: event.target.value }))} required /></label><label>Task title<input value={taskTemplateForm.title} onChange={event => setTaskTemplateForm(current => ({ ...current, title: event.target.value }))} required /></label><label>Description<textarea value={taskTemplateForm.description} onChange={event => setTaskTemplateForm(current => ({ ...current, description: event.target.value }))} /></label><div className="modal-grid"><label>Priority<AppSelect value={taskTemplateForm.priority} onChange={event => setTaskTemplateForm(current => ({ ...current, priority: event.target.value }))}><option value="urgent">Urgent</option><option value="high">High</option><option value="normal">Normal</option><option value="low">Low</option></AppSelect></label><label>Bucket<input value={taskTemplateForm.bucket} onChange={event => setTaskTemplateForm(current => ({ ...current, bucket: event.target.value }))} /></label><label>Repeat<AppSelect value={taskTemplateForm.recurrence} onChange={event => setTaskTemplateForm(current => ({ ...current, recurrence: event.target.value }))}><option value="none">Does not repeat</option><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></AppSelect></label><label>Assignee<AppSelect value={taskTemplateForm.assignee_id} onChange={event => setTaskTemplateForm(current => ({ ...current, assignee_id: event.target.value }))}><option value="">Unassigned</option>{members.map(member => <option key={member.id} value={member.id}>{[member.first_name, member.last_name].filter(Boolean).join(' ') || member.email}</option>)}</AppSelect></label><label>Project<AppSelect value={taskTemplateForm.project_id} onChange={event => setTaskTemplateForm(current => ({ ...current, project_id: event.target.value }))}><option value="">General</option>{projects.map(project => <option key={project.id} value={project.id}>{project.name}</option>)}</AppSelect></label><label>Workstream<input value={taskTemplateForm.workstream} onChange={event => setTaskTemplateForm(current => ({ ...current, workstream: event.target.value }))} /></label></div><button className="secondary-button" disabled={templateSaving}>{templateSaving ? 'Saving…' : 'Create task template'}</button></form><div className="settings-template-list">{taskTemplates.map(template => <div className="settings-template-row" key={template.id}><div><strong>{template.name}</strong><span>{template.title} · {template.priority} · {template.bucket}</span></div><button type="button" className="secondary-button" onClick={() => applyTaskTemplate(template)}>Use</button>{canManageMembers && <button type="button" className="inline-delete" onClick={() => deleteTaskTemplate(template)} aria-label={`Delete ${template.name}`}><X size={14} /></button>}</div>)}</div></div><div className="settings-template-section"><h3>Project templates</h3><form className="settings-template-form" onSubmit={createProjectTemplate}><label>Template name<input value={projectTemplateForm.name} onChange={event => setProjectTemplateForm(current => ({ ...current, name: event.target.value }))} required /></label><label>Project name<input value={projectTemplateForm.project_name} onChange={event => setProjectTemplateForm(current => ({ ...current, project_name: event.target.value }))} required /></label><label>Description<textarea value={projectTemplateForm.description} onChange={event => setProjectTemplateForm(current => ({ ...current, description: event.target.value }))} /></label><label>Due in days<input type="number" min="0" max="365" value={projectTemplateForm.due_days} onChange={event => setProjectTemplateForm(current => ({ ...current, due_days: event.target.value }))} /></label><button className="secondary-button" disabled={templateSaving}>{templateSaving ? 'Saving…' : 'Create project template'}</button></form><div className="settings-template-list">{projectTemplates.map(template => <div className="settings-template-row" key={template.id}><div><strong>{template.name}</strong><span>{template.project_name} · {template.due_days} days</span></div><button type="button" className="secondary-button" onClick={() => applyProjectTemplate(template)}>Use</button>{canManageMembers && <button type="button" className="inline-delete" onClick={() => deleteProjectTemplate(template)} aria-label={`Delete ${template.name}`}><X size={14} /></button>}</div>)}</div></div>{templateError && <p className="auth-error" role="alert">{templateError}</p>}</Card>}{section === 'workspace' && <Card className="settings-panel"><div className="settings-panel-heading"><div><p className="eyebrow">Workspace administration</p><h2>Workspace access</h2><p>Review who can access {currentWorkspace?.name || 'this workspace'}.</p></div></div><div className="settings-stat-grid"><div><strong>{members.length}</strong><span>Members</span></div><div><strong>{members.filter(member => member.role === 'owner' || member.role === 'manager').length}</strong><span>Managers</span></div><div><strong>{currentWorkspace?.role === 'owner' ? 'Owner' : canManageMembers ? 'Manager' : 'Member'}</strong><span>Your role</span></div></div><div className="settings-member-list">{permissionsError && <p className="auth-error" role="alert">{permissionsError}</p>}{members.map(member => {
          ...
          const canEditThisRow = member.role !== 'owner' && (isOwner || (canManageMembers && member.role === 'member'))
          return <div className="settings-member-row" key={member.id}><Avatar name={[member.first_name, member.last_name].filter(Boolean).join(' ') || member.email} avatarUrl={member.avatar_url} presence={member.presence} small /><div><strong>{[member.first_name, member.last_name].filter(Boolean).join(' ') || member.email}</strong><span>{member.email}</span></div>{canEditThisRow ? <AppSelect value={member.role} disabled={permissionsSavingId === member.id} onChange={event => changeMemberRole(member, event.target.value)} aria-label={`Change role for ${member.email}`}><option value="member">Member</option><option value="manager">Manager</option></AppSelect> : <em>{member.role}</em>}</div>
        })}</div>{canManageMembers && <p className="settings-note">Invite new members and remove existing ones from Team board.</p>}{isOwner && members.filter(member => member.role === 'manager').length > 0 && <div className="settings-row settings-control-row" style={{ marginTop: '1rem', flexDirection: 'column', alignItems: 'stretch', gap: '0.75rem' }}><div><strong>Manager permissions</strong><span>Fine-tune exactly what each manager can do. Members always keep the fixed minimal set (create and edit their own tasks, use Zuri, view reports).</span></div>{members.filter(member => member.role === 'manager').map(manager => <div key={manager.id} className="rounded-xl border border-border p-4"><strong className="block text-sm">{[manager.first_name, manager.last_name].filter(Boolean).join(' ') || manager.email}</strong><span className="mt-1 block text-xs text-text-muted">{manager.email}</span><div className="mt-3 grid gap-2 sm:grid-cols-2">{PERMISSION_LABELS.map(([key, label]) => <label key={key} className="flex items-center gap-2 text-xs"><input type="checkbox" checked={(manager.permissions || []).includes(key)} disabled={permissionsSavingId === manager.id} onChange={() => toggleManagerPermission(manager, key)} />{label}</label>)}</div></div>)}</div>}{isOwner && <div className="settings-row settings-control-row" style={{ marginTop: '1rem', flexDirection: 'column', alignItems: 'stretch', gap: '0.75rem' }}><div><strong>Danger zone</strong><span>{isArchived ? 'This workspace is archived. Restore it to resume normal use, or delete it permanently.' : 'Archiving keeps your data but blocks new invitations and stops it from being anyone\'s default.'}</span></div>{lifecycleError && <p className="auth-error" role="alert">{lifecycleError}</p>}<div className="flex flex-wrap gap-2">{!isArchived && <Button type="button" variant="outline" disabled={lifecycleBusy} onClick={archiveWorkspace}>Archive workspace</Button>}{isArchived && <Button type="button" variant="outline" disabled={lifecycleBusy} onClick={restoreWorkspace}>Restore workspace</Button>}</div>{isArchived && <div className="flex flex-wrap items-center gap-2"><label>Type "{currentWorkspace?.name}" to confirm permanent deletion<input value={deleteConfirmText} onChange={event => setDeleteConfirmText(event.target.value)} placeholder={currentWorkspace?.name} /></label><Button type="button" variant="destructive" disabled={lifecycleBusy || deleteConfirmText !== currentWorkspace?.name} onClick={deleteWorkspace}>Delete permanently</Button></div>}</div>}</Card>}
