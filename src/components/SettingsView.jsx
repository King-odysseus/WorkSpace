    ...
    <div className="settings-shell">
      ...
      <div className="settings-content">
        ...
        {section === 'integrations' && <Card className="settings-panel"><div className="settings-panel-heading"><div><p className="eyebrow">Connect other tools</p><h2>Integrations</h2><p>Send WorkSpace notifications to Microsoft Teams or Slack, and subscribe to the team calendar from Outlook or Google Calendar.</p></div></div>
          ...
          {canManageMembers && <form className="settings-webhook-form" onSubmit={addWebhook}><select value={webhookForm.kind} onChange={event => setWebhookForm(current => ({ ...current, kind: event.target.value }))} aria-label="Webhook type"><option value="teams">Microsoft Teams</option><option value="slack">Slack</option><option value="generic">Generic JSON</option></select><input type="url" required placeholder="https://... incoming webhook URL" value={webhookForm.url} onChange={event => setWebhookForm(current => ({ ...current, url: event.target.value }))} aria-label="Webhook URL" /><input type="text" placeholder="Label (optional)" value={webhookForm.label} onChange={event => setWebhookForm(current => ({ ...current, label: event.target.value }))} aria-label="Webhook label" maxLength={120} /><Button type="submit" disabled={webhookSaving}>{webhookSaving ? 'Connecting…' : 'Connect'}</Button></form>}
