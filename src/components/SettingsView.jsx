  ...
  return <section className="workspace-view settings-view">
    ...
    <div className="settings-shell">
      ...
      <div className="settings-content">
        ...
        {section === 'profile' && workspaces.length > 1 && <Card className="settings-panel"><div className="settings-panel-heading"><div><p className="eyebrow">Workspace entry</p><h2>Default workspace</h2><p>Choose the workspace WorkSpace opens when you sign in.</p></div></div><div className="grid gap-3 sm:grid-cols-2">{workspaces.map(workspace => <button type="button" key={workspace.id} onClick={() => onSetDefaultWorkspace?.(workspace.id)} aria-pressed={workspace.id === defaultWorkspaceId} className={`rounded-xl border p-4 text-left transition-colors ${workspace.id === defaultWorkspaceId ? 'border-primary bg-primary/5' : 'border-border hover:bg-surface-secondary'}`}><strong className="block text-sm">{workspace.name}</strong><span className="mt-1 block text-xs text-text-muted">{workspace.id === defaultWorkspaceId ? 'Default entry workspace' : 'Set as default'}</span></button>)}</div></Card>}
