...
import { CheckCircle2, CircleAlert, LoaderCircle, Mail, Moon, Sun, WifiOff, X } from 'lucide-react'
...
function AuthScreen({ theme, onToggleTheme, onAuthenticated, connectionError, inviteInfo }) {
  ...
  return <div className="auth-screen auth-layout"><button type="button" className="auth-theme-toggle" onClick={onToggleTheme} aria-label={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`} aria-pressed={theme === 'dark'}>{theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}</button><aside className="auth-brand-panel"><div className="auth-brand"><span className="auth-brand-mark">W</span><span>Workspace</span></div><div className="auth-brand-copy"><h2>Team operations, planning and collaboration in one place.</h2><ul><li><CheckCircle2 size={18} />Plan work across shared buckets</li><li><CheckCircle2 size={18} />Track workload, follow-ups and check-ins</li><li><CheckCircle2 size={18} />Report on progress across every project</li></ul></div><p className="auth-brand-footer">Copyright 2025 Workspace | Privacy | Terms</p></aside><main className="auth-form-panel"><div className="auth-form-inner"><p className="eyebrow">{mode === 'login' ? 'Sign in' : 'Create account'}</p><h1>{mode === 'login' ? 'Sign in' : 'Create your workspace'}</h1><p className="auth-subtitle">{mode === 'login' ? 'Welcome back. Sign in to continue to your workspace.' : 'Bring your team, tasks, and follow-ups into one calm workspace.'}</p>{inviteInfo && <p className="auth-invite-banner"><Mail size={17} />You have been invited to join <strong>{inviteInfo.workspace_name}</strong> as a {inviteInfo.role}.</p>}<form onSubmit={submit}>{mode === 'signup' && <><label>First name<input name="first_name" value={form.first_name} onChange={updateField} placeholder="Your first name" required /></label>{!inviteInfo && <label>Workspace name<input name="workspace_name" value={form.workspace_name} onChange={updateField} placeholder="Your team or company" required /></label>}</>}<label>Email<input name="email" type="email" value={form.email} onChange={updateField} placeholder="you@company.com" readOnly={Boolean(inviteInfo?.email)} required /></label><div className="auth-password-label"><span>Password</span>{mode === 'login' && <button type="button" className="auth-forgot" onClick={() => setError('Password reset is available from your workspace administrator.')}>Forgot?</button>}<input aria-label="Password" name="password" type="password" value={form.password} onChange={updateField} placeholder="At least 8 characters" minLength="8" required /></div>{error && <p className="auth-error">{error}</p>}{connectionError && !error && <p className="auth-error">The API is unavailable. Start Django on port 8000.</p>}<button type="submit" className="primary-button auth-submit" disabled={submitting}>{submitting ? <><LoaderCircle size={16} className="auth-spinner" /> Signing in...</> : mode === 'login' ? 'Sign in' : 'Create workspace'}</button></form><GoogleSignInButton onCredential={submitGoogleCredential} theme={theme} mode={mode} /><button type="button" className="auth-switch" onClick={() => { setMode(current => current === 'login' ? 'signup' : 'login'); setError('') }}>{mode === 'login' ? 'New to Workspace? Create an account' : 'Already have an account? Sign in'}</button><p className="auth-subtitle auth-policy-links"><a href="/privacy-policy">Privacy policy</a> | <a href="/terms-of-service">Terms of service</a></p></div></main><aside className="auth-states-panel" aria-label="Important authentication states"><p className="auth-states-title">Important states</p><p className="auth-state-label">Submitting</p><div className="auth-state auth-state-loading"><LoaderCircle size={17} className="auth-spinner" />Signing in...</div><p className="auth-state-label">Authentication failed</p><div className="auth-state auth-state-error"><CircleAlert size={17} /><span><strong>Incorrect email or password</strong><small>Try again, or reset your password.</small></span></div><p className="auth-state-label">Backend unavailable</p><div className="auth-state auth-state-warning"><WifiOff size={17} /><span><strong>Can't reach Workspace</strong><small>Check your connection, then retry.</small><small>Retry</small></span></div><p className="auth-state-label">Successful authentication</p><div className="auth-state auth-state-success"><CheckCircle2 size={17} /><span><strong>Signed in</strong><small>Opening your workspace...</small></span></div></aside></div>
}
