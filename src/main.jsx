  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="main-content">
      ...
      <div className="page-content">
        ...
        {active !== 'Today' && <WorkspaceView active={active} data={workspaceData} tasks={tasks} searchQuery={searchQuery} onSearchChange={setSearchQuery} theme={theme} sidebarCollapsed={sidebarCollapsed} workspaceId={workspaceId} currentUserName={[session.user.first_name, session.user.last_name].filter(Boolean).join(' ') || session.user.email} canManageMembers={['owner', 'manager'].includes(currentWorkspace?.role)} canManageTasks={['owner', 'manager'].includes(currentWorkspace?.role)} onToggleTheme={() => setTheme(current => current === 'dark' ? 'light' : 'dark')} onToggleSidebar={() => setSidebarCollapsed(current => !current)} onComplete={completeTask} onStatusChange={changeTaskStatus} onBucketChange={changeTaskBucket} onDelete={deleteTask} onAddTask={() => setShowModal(true)} onOpenTask={setSelectedTask} />}
        {active === 'Today' && <>
...
function WorkspaceView({ active, data, tasks, searchQuery, onSearchChange, theme, sidebarCollapsed, workspaceId, currentUserName, canManageMembers, canManageTasks, onToggleTheme, onToggleSidebar, onComplete, onStatusChange, onBucketChange, onDelete, onAddTask, onOpenTask }) {
  const today = new Date().toISOString().slice(0, 10)
  ...
  const openComposer = type => {
    ...
    setComposerError('')
    setForm(current => ({ ...current, title: '', name: '', description: '', start_at: '', end_at: '', reminder_minutes: 15, completed: '', next_steps: '', blockers: '', message: '', channel: chatChannel, note: '', due_date: '', assigned_to: '', date: today, email: '', role: 'member' }))
  ...
  if (active === 'Projects') {
    return <section className="workspace-view"><WorkspaceViewHeading title={title} subtitle={subtitle} action="New project" onAction={() => openComposer('project')} /><div className="project-grid">{localData.projects.length ? localData.projects.map(project => <article className="workspace-card project-card" key={project.id}><div className="project-card-top"><span className="project-icon"><Target size={17} /></span><select className={`project-status ${project.status}`} value={project.status} onChange={event => updateProjectStatus(project, event.target.value)} aria-label={`Change status for ${project.name}`}><option value="planning">Planning</option><option value="active">Active</option><option value="paused">Paused</option><option value="completed">Completed</option></select>{canManageMembers && <button className="inline-delete" onClick={() => deleteProject(project)} aria-label={`Delete ${project.name}`}><X size={14} /></button>}</div><h3>{project.name}</h3><p>{project.description || 'No project description yet.'}</p><div className="project-footer"><span>{project.due_date ? `Due ${project.due_date}` : 'No due date'}</span><ArrowUpRight size={15} /></div></article>) : <EmptyState text="No projects have been created yet." />}</div>{composerOpen && <WorkspaceComposer type="project" form={form} setForm={setForm} error={composerError} submitting={submitting} onClose={() => setComposerOpen(false)} onSubmit={submitComposer} />}</section>
  }
...
function WorkspaceComposer({ type, form, setForm, replyTo, error, submitting, onClose, onSubmit, members = [] }) {
