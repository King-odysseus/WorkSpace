...
function App() {
  ...
  const addTask = async event => {
    ...
    try {
      ...
      setTasks(current => [...current, { id: data.task.id, title: data.task.title, member: data.task.assignee_name || 'Unassigned', tag: data.task.project || 'General', status: 'todo', priority: data.task.priority || 'normal', due: data.task.due_date || 'No due date', estimate: 'n/a', recurrence: data.task.recurrence || 'none', bucket: data.task.bucket || 'Backlog', labels: data.task.labels || [] }])
      setNewTask('')
      setNewAssigneeId('')
      setNewProjectId('')
      setNewDueDate('')
