...
import {
  ClockInCard, MyTasksView, ProjectCostBudgetPanel, ProjectOperationsSummary, ProjectProgress, ProjectRiskIssuePanel,
...
function App() {
  ...
  useEffect(() => {
    ...
    const refreshCollaboration = () => {
      ...
      return Promise.all([
      readAllTasks(),
      read(`/api/workspaces/${workspaceId}/members/?page_size=500`, { members: [] }),
      read(`/api/workspaces/${workspaceId}/projects/?page_size=500`, { projects: [] }),
      read(`/api/workspaces/${workspaceId}/lookup-values/`, { lookup_values: [] }),
      read(`/api/workspaces/${workspaceId}/task-templates/?page_size=500`, { task_templates: [] }),
      read(`/api/workspaces/${workspaceId}/project-templates/?page_size=500`, { project_templates: [] }),
      ...
      read(`/api/workspaces/${workspaceId}/invitations/?page_size=500`, { invitations: [] }),
...
function WorkspaceView({ active, data, tasks, searchQuery, onSearchChange, onNavigate, theme, onSetTheme, sidebarCollapsed, workspaceId, currentWorkspace, currentUserName, currentUserEmail, currentUserId, currentUserAvatarUrl, currentUserPresence, currentUserWorkspaces, defaultWorkspaceId, onSetDefaultWorkspace, onProfileUpdated, canManageMembers, canManageTasks, reportRange, setReportRange, shiftLogUserId, setShiftLogUserId, shiftLogPage, setShiftLogPage, reportLastUpdated, onToggleTheme, onToggleSidebar, onComplete, onStatusChange, onBucketChange, onDelete, onAddTask, onOpenTask, onActionError, onRefresh, onConfirm }) {
  ...
  if (active === 'Projects') {
    ...
    if (selectedProjectWorkspace) {
      const openOperation = operation => { setProjectOperation(operation); window.history.replaceState(null, '', `#project/${selectedProjectWorkspace.id}/${operation}`) }
      const closeProject = () => { setProjectOperation(''); setSelectedProjectWorkspace(null); window.history.replaceState(null, '', window.location.pathname) }
      const operationTitle = { budget: 'Budget & costs', resources: 'Resources', risks: 'Risks & issues' }[projectOperation]
      return <section className="workspace-view project-detail-view"><button type="button" className="text-button project-back-button" onClick={projectOperation ? () => setProjectOperation('') : closeProject}>← {projectOperation ? 'Back to project overview' : 'Back to projects'}</button><WorkspaceViewHeading title={projectOperation ? `${selectedProjectWorkspace.name}: ${operationTitle}` : selectedProjectWorkspace.name} subtitle={projectOperation ? 'Detailed project operational controls.' : (selectedProjectWorkspace.description || 'Project workspace and delivery controls.')} action={canManageMembers ? 'Edit project' : undefined} onAction={() => setSelectedProject(selectedProjectWorkspace)} />{!projectOperation && <><div className="project-detail-links"><button type="button" onClick={() => { window.dispatchEvent(new CustomEvent('planner:project', { detail: String(selectedProjectWorkspace.id) })); onNavigate('Planner') }}><strong>Planner</strong><span>Open tasks for this project</span></button></div><ProjectOperationsSummary project={selectedProjectWorkspace} workspaceId={workspaceId} onOpen={openOperation} /></>}{projectOperation === 'risks' && <ProjectRiskIssuePanel projects={[selectedProjectWorkspace]} workspaceId={workspaceId} tasks={tasks} canManage={canManageMembers} />}{projectOperation === 'resources' && <ProjectStakeholderResourcePanel project={selectedProjectWorkspace} workspaceId={workspaceId} canManage={canManageMembers} tasks={tasks} />}{projectOperation === 'budget' && <ProjectCostBudgetPanel project={selectedProjectWorkspace} workspaceId={workspaceId} canManage={canManageMembers} onProjectUpdated={updatedProject => { setSelectedProjectWorkspace(updatedProject); setLocalData(current => ({ ...current, projects: current.projects.map(item => item.id === updatedProject.id ? updatedProject : item) })); onRefresh() }} />}{selectedProject && <ProjectEditDrawer project={selectedProject} workspaceId={workspaceId} onClose={() => setSelectedProject(null)} onUpdated={updatedProject => { setSelectedProjectWorkspace(updatedProject); setLocalData(current => ({ ...current, projects: current.projects.map(item => item.id === updatedProject.id ? updatedProject : item) })); onRefresh(); setSelectedProject(null) }} />}</section>
    }
