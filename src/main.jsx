...
function App() {
  ...
  useEffect(() => {
    ...
    const refreshCollaboration = () => Promise.all([
      ...
      auditRequest,
    ]).then(([taskData, memberData, projectData, messageData, followUpData, eventData, checkInData, notificationData, activityData, bucketData, invitationData, savedViewData, reportData, auditData]) => {
      if (!isCurrent) return
  ...
  const mapApiTask = apiTask => {
    ...
    return { id: apiTask.id, title: apiTask.title, description: apiTask.description || '', member: apiTask.assignee_name || 'Unassigned', tag: apiTask.project || 'General', status: apiTask.status === 'in_progress' ? 'in progress' : apiTask.status, priority: apiTask.priority || 'normal', due: taskDueLabel(apiTask.due_date, today), due_date: apiTask.due_date || '', estimate: 'n/a', assignee_id: apiTask.assignee_id || '', project_id: apiTask.project_id || '', can_edit: ['owner', 'manager'].includes(workspaceRole) || apiTask.assignee_id === session.user.id, recurrence: apiTask.recurrence || 'none', bucket: apiTask.bucket || 'Backlog', labels: apiTask.labels || [] }
  }
  ...
  const addTask = async event => {
    ...
    try {
      ...
      setTasks(current => [...current, { id: data.task.id, title: data.task.title, description: data.task.description || '', member: data.task.assignee_name || 'Unassigned', tag: data.task.project || 'General', status: 'todo', priority: data.task.priority || 'normal', due: taskDueLabel(data.task.due_date, today), due_date: data.task.due_date || '', estimate: 'n/a', assignee_id: data.task.assignee_id || '', project_id: data.task.project_id || '', can_edit: ['owner', 'manager'].includes(currentWorkspace?.role) || data.task.assignee_id === session.user.id, recurrence: data.task.recurrence || 'none', bucket: data.task.bucket || 'Backlog', labels: data.task.labels || [] }])
      setNewTask('')
      setNewDescription('')
      setNewAssigneeId('')
      setNewProjectId('')
  ...
  const navItems = [
    ...
    { label: 'Team board', icon: Users }, { label: 'Planner', icon: LayoutGrid }, { label: 'Calendar', icon: CalendarDays }, { label: 'Reports', icon: BarChart3 }, { label: 'Activity', icon: Clock3 },
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <aside className="sidebar">
      ...
      <nav className="main-nav">
        ...
        <button type="button" className={`nav-item ${active === 'Follow-up' ? 'active' : ''}`} onClick={() => setActive('Follow-up')} title={sidebarCollapsed ? 'Follow-up' : undefined} aria-current={active === 'Follow-up' ? 'page' : undefined}><Bell size={18} /><span>Follow-up</span>{workspaceData.followUps.filter(item => item.status !== 'completed').length > 0 && <span className="nav-badge alert">{workspaceData.followUps.filter(item => item.status !== 'completed').length}</span>}</button>
        <button type="button" className={`nav-item ${active === 'Check-ins' ? 'active' : ''}`} onClick={() => setActive('Check-ins')} title={sidebarCollapsed ? 'Check-ins' : undefined} aria-current={active === 'Check-ins' ? 'page' : undefined}><Hash size={18} /><span>Check-ins</span></button>
      </nav>
      <div className="sidebar-bottom"><div className="upgrade-card"><Sparkles size={16} /><div><strong>Make your week flow</strong><span>Set your priorities</span></div><ArrowUpRight size={15} /></div><button className={`nav-item ${active === 'Settings' ? 'active' : ''}`} onClick={() => setActive('Settings')} title={sidebarCollapsed ? 'Settings' : undefined}><Settings size={18} /><span>Settings</span></button><div className="profile"><div className="avatar navy">{currentUserInitials}</div><div><strong>{currentUserName}</strong><span>{currentWorkspace?.role || 'Member'}</span></div><MoreHorizontal size={17} /></div></div>
    </aside>
    ...
    <main id="main-content" className="main-content" tabIndex="-1">
      ...
      <div className="page-content">
        ...
        <div className="content-grid">
          <section className="board-section"><div className="section-header"><div><h2>Team pulse</h2><p>Today's commitments across your team</p></div><div className="header-actions"><button className="filter-button" onClick={() => setSelectedFilter(current => current === 'All work' ? 'in progress' : 'All work')}><Filter size={15} /> {selectedFilter === 'All work' ? 'Filters' : selectedFilter} <ChevronDown size={14} /></button><button className="more-button" onClick={() => setActive('Planner')} aria-label="Open planner"><MoreHorizontal size={19} /></button></div></div><div className="board-tabs"><button type="button" className={`tab ${teamBoardMode === 'people' ? 'active' : ''}`} onClick={() => setTeamBoardMode('people')} aria-pressed={teamBoardMode === 'people'}>People</button><button type="button" className={`tab ${teamBoardMode === 'status' ? 'active' : ''}`} onClick={() => setTeamBoardMode('status')} aria-pressed={teamBoardMode === 'status'}>Status</button><button type="button" className={`tab ${teamBoardMode === 'priority' ? 'active' : ''}`} onClick={() => setTeamBoardMode('priority')} aria-pressed={teamBoardMode === 'priority'}>Priority</button><label className="filter-select"><span className="status-dot all" /><select value={selectedFilter} onChange={event => setSelectedFilter(event.target.value)}><option>All work</option><option value="todo">To do</option><option value="in progress">In progress</option><option value="review">Review</option><option value="blocked">Blocked</option><option value="done">Done</option></select></label></div><div className="team-board">{teamBoardMode === 'people' && teamBoardMembers.map(member => { const memberTasks = member.id === 'unassigned' ? visibleTasks.filter(task => task.member === 'Unassigned') : visibleTasks.filter(task => task.member === member.name); return <div className="member-row" key={member.name}><div className="member-cell"><Avatar member={member} /><div><strong>{member.name}</strong><span>{member.role}</span></div></div><div className="task-stack">{memberTasks.length ? memberTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} canDelete={canManageTasks} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => openTaskModal(member.id === 'unassigned' ? undefined : member.id)} aria-label={`Add task for ${member.name}`}><Plus size={16} /></button></div> })}{teamBoardMode === 'status' && ['todo', 'in progress', 'review', 'blocked', 'done'].map(status => { const statusTasks = visibleTasks.filter(task => task.status === status); return <div className="member-row" key={status}><div className="member-cell"><span className="status-dot all" /><div><strong>{status}</strong><span>{statusTasks.length} tasks</span></div></div><div className="task-stack">{statusTasks.length ? statusTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} canDelete={canManageTasks} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => openTaskModal()} aria-label={`Add ${status} task`}><Plus size={16} /></button></div> })}{teamBoardMode === 'priority' && ['urgent', 'high', 'normal', 'low'].map(priority => { const priorityTasks = visibleTasks.filter(task => task.priority === priority); return <div className="member-row" key={priority}><div className="member-cell"><span className="status-dot all" /><div><strong>{priority} priority</strong><span>{priorityTasks.length} tasks</span></div></div><div className="task-stack">{priorityTasks.length ? priorityTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} canDelete={canManageTasks} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => openTaskModal()} aria-label={`Add ${priority} priority task`}><Plus size={16} /></button></div> })}</div><button className="add-person" onClick={() => canManageMembers ? openComposer('invite') : setActive('Team board')}><Plus size={16} /> {canManageMembers ? 'Invite team member' : 'View team members'}</button></section>
...
function WorkspaceView({ active, data, tasks, searchQuery, onSearchChange, onNavigate, theme, sidebarCollapsed, workspaceId, currentUserName, currentUserId, canManageMembers, canManageTasks, onToggleTheme, onToggleSidebar, onComplete, onStatusChange, onBucketChange, onDelete, onAddTask, onOpenTask, onActionError, onRefresh }) {
  const today = toDateKey(new Date())
  ...
  if (active === 'Planner') {
    ...
    const filteredTasks = tasks.filter(task => {
      ...
      const matchesMember = plannerFilter.startsWith('member:') && String(task.assignee_id || '') === plannerFilter.slice(7)
      const matchesFilter = plannerFilter === 'all' || task.status === plannerFilter || task.bucket === plannerFilter || (plannerFilter === 'mine' && task.member === currentUserName) || (task.labels || []).includes(plannerFilter) || matchesMember
  ...
  if (active === 'Projects') {
    return <section className="workspace-view"><WorkspaceViewHeading title={title} subtitle={subtitle} action={canManageMembers ? 'New project' : undefined} onAction={() => openComposer('project')} /><div className="project-grid">{localData.projects.length ? localData.projects.map(project => <article className="workspace-card project-card" key={project.id}><div className="project-card-top"><span className="project-icon"><Target size={17} /></span><select className={`project-status ${project.status}`} value={project.status} onChange={event => updateProjectStatus(project, event.target.value)} disabled={!canManageMembers} aria-label={`Change status for ${project.name}`}><option value="planning">Planning</option><option value="active">Active</option><option value="paused">Paused</option><option value="completed">Completed</option></select>{canManageMembers && <><button type="button" className="inline-delete" onClick={() => setSelectedProject(project)} aria-label={`Edit ${project.name}`}><MoreHorizontal size={14} /></button><button type="button" className="inline-delete" onClick={() => deleteProject(project)} aria-label={`Delete ${project.name}`}><X size={14} /></button></>}</div><h3>{project.name}</h3><p>{project.description || 'No project description yet.'}</p><ProjectProgress project={project} tasks={tasks} /><div className="project-footer"><span>{project.due_date ? `Due ${project.due_date}` : 'No due date'}</span><button type="button" className="project-task-link" onClick={() => { onSearchChange(project.name); onNavigate('Planner') }} aria-label={`View tasks for ${project.name}`}>View tasks <ArrowUpRight size={15} /></button></div></article>) : <EmptyState text="No projects have been created yet." />}</div>{composerOpen && <WorkspaceComposer type="project" form={form} setForm={setForm} error={composerError} submitting={submitting} onClose={() => setComposerOpen(false)} onSubmit={submitComposer} />}{selectedProject && <ProjectEditDrawer project={selectedProject} workspaceId={workspaceId} onClose={() => setSelectedProject(null)} onUpdated={updatedProject => { setLocalData(current => ({ ...current, projects: current.projects.map(item => item.id === updatedProject.id ? updatedProject : item) })); onRefresh(); setSelectedProject(null) }} />}</section>
  }
...
function WorkspaceComposer({ type, form, setForm, replyTo, error, submitting, onClose, onSubmit, members = [], tasks = [], channels = ['general'] }) {
  ...
  const field = (name, label, placeholder, inputType = 'text') => {
    ...
    return <>{baseField}<label>Description<textarea name="description" value={form.description} onChange={update} placeholder="What is this event for?" maxLength="4000" /></label><label>Event type<select name="event_type" value={form.event_type} onChange={update}><option value="meeting">Meeting</option><option value="focus">Focus time</option><option value="deadline">Deadline</option><option value="reminder">Reminder</option></select></label></>
  }
