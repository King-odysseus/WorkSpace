...
function App() {
  ...
  const addTask = async event => {
    ...
    try {
      ...
      setTasks(current => [...current, { id: data.task.id, title: data.task.title, member: data.task.assignee_name || 'Unassigned', tag: data.task.project || 'General', status: 'todo', priority: data.task.priority || 'normal', due: taskDueLabel(data.task.due_date, today), due_date: data.task.due_date || '', estimate: 'n/a', assignee_id: data.task.assignee_id || '', project_id: data.task.project_id || '', can_edit: ['owner', 'manager'].includes(currentWorkspace?.role) || data.task.assignee_id === session.user.id, recurrence: data.task.recurrence || 'none', bucket: data.task.bucket || 'Backlog', labels: data.task.labels || [] }])
      setNewTask('')
      setNewAssigneeId('')
      setNewProjectId('')
