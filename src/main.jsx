...
function TaskDetailDrawer({ task, workspaceId, onClose }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={onClose}><aside className="task-drawer" onMouseDown={event => event.stopPropagation()}><div className="drawer-heading"><div><p className="eyebrow">Task details</p><h2>{task.title}</h2><span>{task.member} | {task.due}</span></div><button className="close-button" onClick={onClose} aria-label="Close task details"><X size={18} /></button></div>{error && <p className="auth-error">{error}</p>}{loading ? <p className="drawer-muted">Loading task details...</p> : <><section className="drawer-section"><div className="drawer-section-heading"><h3>Subtasks</h3><span>{subtasks.filter(item => item.completed).length} of {subtasks.length}</span></div>{subtasks.map(item => <label className="subtask-row" key={item.id}><input type="checkbox" checked={item.completed} onChange={() => toggleSubtask(item)} /><span className={item.completed ? 'completed' : ''}>{item.title}</span></label>)}<form className="inline-form" onSubmit={addSubtask}><input value={subtask} onChange={event => setSubtask(event.target.value)} placeholder="Add a subtask" aria-label="Add a subtask" /><button className="secondary-button">Add</button></form></section><section className="drawer-section"><div className="drawer-section-heading"><h3>Comments</h3><span>{comments.length}</span></div>{comments.length ? comments.map(item => <article className="drawer-comment" key={item.id}><strong>{item.author_name}</strong><p>{item.body}</p></article>) : <p className="drawer-muted">No comments yet.</p>}<form className="drawer-comment-form" onSubmit={addComment}><textarea value={comment} onChange={event => setComment(event.target.value)} placeholder="Write an update for the team" aria-label="Write a task comment" /><button className="primary-button">Post comment</button></form></section></>}</aside></div>
}
