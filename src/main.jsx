...
function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main className="main-content">
      ...
      <div className="page-content">
        ...
        <div className="content-grid">
          <section className="board-section"><div className="section-header"><div><h2>Team pulse</h2><p>Today's commitments across your team</p></div><div className="header-actions"><button className="filter-button" onClick={() => setSelectedFilter(current => current === 'All work' ? 'in progress' : 'All work')}><Filter size={15} /> {selectedFilter === 'All work' ? 'Filters' : selectedFilter} <ChevronDown size={14} /></button><button className="more-button" onClick={() => setActive('Planner')} aria-label="Open planner"><MoreHorizontal size={19} /></button></div></div><div className="board-tabs"><button className={`tab ${teamBoardMode === 'people' ? 'active' : ''}`} onClick={() => setTeamBoardMode('people')}>People</button><button className={`tab ${teamBoardMode === 'status' ? 'active' : ''}`} onClick={() => setTeamBoardMode('status')}>Status</button><button className={`tab ${teamBoardMode === 'priority' ? 'active' : ''}`} onClick={() => setTeamBoardMode('priority')}>Priority</button><label className="filter-select"><span className="status-dot all" /><select value={selectedFilter} onChange={event => setSelectedFilter(event.target.value)}><option>All work</option><option value="todo">To do</option><option value="in progress">In progress</option><option value="review">Review</option><option value="blocked">Blocked</option><option value="done">Done</option></select></label></div><div className="team-board">{teamBoardMode === 'people' && teamMembers.map(member => { const memberTasks = visibleTasks.filter(task => task.member === member.name); return <div className="member-row" key={member.name}><div className="member-cell"><Avatar member={member} /><div><strong>{member.name}</strong><span>{member.role}</span></div></div><div className="task-stack">{memberTasks.length ? memberTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => setShowModal(true)} aria-label={`Add task for ${member.name}`}><Plus size={16} /></button></div> })}{teamBoardMode === 'status' && ['todo', 'in progress', 'review', 'blocked', 'done'].map(status => { const statusTasks = visibleTasks.filter(task => task.status === status); return <div className="member-row" key={status}><div className="member-cell"><span className="status-dot all" /><div><strong>{status}</strong><span>{statusTasks.length} tasks</span></div></div><div className="task-stack">{statusTasks.length ? statusTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => setShowModal(true)} aria-label={`Add ${status} task`}><Plus size={16} /></button></div> })}{teamBoardMode === 'priority' && ['urgent', 'high', 'normal', 'low'].map(priority => { const priorityTasks = visibleTasks.filter(task => task.priority === priority); return <div className="member-row" key={priority}><div className="member-cell"><span className="status-dot all" /><div><strong>{priority} priority</strong><span>{priorityTasks.length} tasks</span></div></div><div className="task-stack">{priorityTasks.length ? priorityTasks.map(task => <TaskCard key={task.id} task={task} onComplete={completeTask} onStatusChange={changeTaskStatus} onDelete={deleteTask} onOpenTask={setSelectedTask} />) : <div className="empty-task">No tasks in this view</div>}</div><button className="row-add" onClick={() => setShowModal(true)} aria-label={`Add ${priority} priority task`}><Plus size={16} /></button></div> })}</div><button className="add-person" onClick={() => setActive('Team board')}><Plus size={16} /> Add team member</button></section>
    ...
    {selectedTask && <TaskDetailDrawer task={selectedTask} workspaceId={activeWorkspaceId} onClose={() => setSelectedTask(null)} onTaskUpdated={updatedTask => setTasks(current => current.map(item => item.id === updatedTask.id ? { ...item, status: updatedTask.status === 'in_progress' ? 'in progress' : updatedTask.status, priority: updatedTask.priority || 'normal', due: updatedTask.due_date || 'No due date', due_date: updatedTask.due_date || '', recurrence: updatedTask.recurrence || 'none' } : item))} />}
  </div>
}
...
function WorkspaceView({ active, data, tasks, searchQuery, onSearchChange, theme, sidebarCollapsed, workspaceId, currentUserName, canManageMembers, onToggleTheme, onToggleSidebar, onComplete, onStatusChange, onBucketChange, onDelete, onAddTask, onOpenTask }) {
  ...
  const openComposer = type => {
    ...
    setComposerError('')
    setForm(current => ({ ...current, title: '', name: '', description: '', start_at: '', end_at: '', reminder_minutes: 15, completed: '', next_steps: '', blockers: '', message: '', channel: chatChannel, note: '', due_date: '', date: today, email: '', role: 'member' }))
  ...
  const submitComposer = async event => {
    ...
    const payloads = { calendar: { title: form.title, description: form.description, start_at: form.start_at, end_at: form.end_at, event_type: form.event_type, reminder_minutes: Number(form.reminder_minutes) }, project: { name: form.name, description: form.description }, checkin: { date: form.date, completed: form.completed, next_steps: form.next_steps, blockers: form.blockers }, chat: { channel: form.channel, message: form.message, parent_id: replyTo?.id || null }, followup: { note: form.note, due_date: form.due_date || null }, invite: { email: form.email, role: form.role } }
    try {
...
function WorkspaceComposer({ type, form, setForm, replyTo, error, submitting, onClose, onSubmit }) {
...
function TaskDetailDrawer({ task, workspaceId, onClose, onTaskUpdated }) {
  ...
  const [taskFields, setTaskFields] = useState({ status: task.status === 'in progress' ? 'in_progress' : task.status, priority: task.priority || 'normal', due_date: task.due_date || '', recurrence: task.recurrence || 'none' })
