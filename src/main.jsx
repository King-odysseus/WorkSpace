...
function App() {
  ...
  const addTask = async event => {
    ...
    try {
      ...
      setTasks(current => [...current, { id: data.task.id, title: data.task.title, member: data.task.assignee_name || 'Unassigned', tag: data.task.project || 'General', status: 'todo', priority: 'normal', due: data.task.due_date || 'No due date', estimate: 'n/a', recurrence: data.task.recurrence || 'none', bucket: data.task.bucket || 'Backlog', labels: data.task.labels || [] }])
      setNewTask('')
      setNewAssigneeId('')
      setNewProjectId('')
      setNewDueDate('')
...
function WorkspaceView({ active, data, tasks, searchQuery, theme, sidebarCollapsed, workspaceId, currentUserName, canManageMembers, onToggleTheme, onToggleSidebar, onComplete, onStatusChange, onBucketChange, onDelete, onAddTask, onOpenTask }) {
  ...
  const [form, setForm] = useState({ title: '', name: '', description: '', start_at: '', end_at: '', event_type: 'meeting', reminder_minutes: 15, completed: '', next_steps: '', blockers: '', message: '', note: '', due_date: '', date: today, email: '', role: 'member' })
  ...
  const openComposer = type => {
    ...
    setComposerError('')
    setForm(current => ({ ...current, title: '', name: '', description: '', start_at: '', end_at: '', reminder_minutes: 15, completed: '', next_steps: '', blockers: '', message: '', note: '', due_date: '', date: today, email: '', role: 'member' }))
  ...
  if (active === 'Chat') {
    return <section className="workspace-view"><WorkspaceViewHeading title={title} subtitle={subtitle} action="New message" onAction={() => openComposer('chat')} /><div className="chat-layout"><div className="workspace-card chat-feed">{localData.messages.length ? localData.messages.map(message => <div className={`chat-message ${message.parent_id ? 'chat-reply' : ''}`} key={message.id}><span className="avatar blue small">{message.author_name.slice(0, 2).toUpperCase()}</span><div><div className="chat-message-meta"><strong>{message.author_name}</strong><span>#{message.channel}</span></div><p>{message.message}</p>{!message.parent_id && <button className="chat-reply-button" onClick={() => { setReplyTo(message); openComposer('chat') }}>Reply{message.reply_count ? ` (${message.reply_count})` : ''}</button>}</div></div>) : <div className="chat-placeholder"><div className="chat-placeholder-icon"><MessageSquare size={22} /></div><h2>No messages yet</h2><p>Start a conversation to keep decisions close to the work.</p></div>}</div><aside className="workspace-side-card"><h3>Channels</h3><div className="channel-row active"><Hash size={15} /> general <span>{localData.messages.length}</span></div><div className="channel-row"><Hash size={15} /> announcements</div><div className="channel-row"><Hash size={15} /> project-launch</div></aside></div>{composerOpen && <WorkspaceComposer type="chat" form={form} setForm={setForm} replyTo={replyTo} error={composerError} submitting={submitting} onClose={() => { setComposerOpen(false); setReplyTo(null) }} onSubmit={submitComposer} />}</section>
  }
