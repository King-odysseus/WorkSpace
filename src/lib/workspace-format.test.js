...
describe('taskDueLabel', () => {
  it('reports overdue only for dates strictly before today', () => {
    ...
    expect(taskDueLabel('2026-09-05', '2026-09-05')).toBe('05-09-26')
    expect(taskDueLabel('', '2026-09-05')).toBe('No due date')
  })
})

describe('formatDay', () => {
  it('writes a day as DD-MM-YY', () => {
    expect(formatDay('2026-09-05')).toBe('05-09-26')
  })
  ...
  it('takes the day off a timestamp without shifting it', () => {
    ...
    expect(formatDay('2026-09-05T23:30:00Z')).toBe('05-09-26')
  })
...
describe('formatDate', () => {
  ...
  it('writes a timestamp in the reader own time zone as DD-MM-YY', () => {
    expect(formatDate(new Date(2026, 8, 5, 23, 30))).toBe('05-09-26')
  })
...
describe('formatDateTime', () => {
  it('puts a 24 hour clock beside the day', () => {
    expect(formatDateTime(new Date(2026, 8, 5, 9, 5))).toBe('05-09-26 09:05')
    expect(formatDateTime(new Date(2026, 8, 5, 18, 45))).toBe('05-09-26 18:45')
  })
...
describe('formatLastSeen', () => {
  it('gives the day in the app wide format once relative wording runs out', () => {
    expect(formatLastSeen(new Date(2020, 0, 5, 12).toISOString())).toBe('Last seen 05-01-20')
  })
})
