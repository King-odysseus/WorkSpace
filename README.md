...
- **Transactional email (Brevo)**: set `BREVO_API_KEY` (from Brevo > SMTP & API > API Keys) to send workspace invitation emails and task/calendar reminder emails. Without it, mail is printed to the console instead of sent. Also set `BREVO_SENDER_EMAIL` and `BREVO_SENDER_NAME` to the sender Brevo has verified for your account - they take priority over `WORKSPACE_DEFAULT_FROM_EMAIL` - and `WORKSPACE_FRONTEND_BASE_URL` (used to build the accept-invite link in the email).
